...
import { Attachment, TechnicalNote, User } from "@prisma/client";
...
export default function MessageBox({ note }: MessageBoxProps) {
  const createdDuration = calculateOpenedDuration(note.createdAt);
            ...
            <div className="w-full">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-x-1 text-xs text-muted-foreground md:text-sm">
                  <h5>Date:</h5>
                  <span>
                    {format(new Date(note.createdAt), "LLLL d, yyyy, h:mm aa")}
                  </span>
                </div>
                {/* Attachment count. */}
                {!!note.attachments && !!note.attachments.length && (
                  <div className="flex items-center text-xs text-emerald-600 md:text-sm">
                    ...
                    <span>{`${note.attachments.length} ${
                      note.attachments.length === 1 ? "file" : "files"
                    }`}</span>
                  </div>
                )}
              </div>
